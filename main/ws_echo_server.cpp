...
{
    struct async_resp_arg* resp_arg = reinterpret_cast<struct async_resp_arg*> (malloc(sizeof(struct async_resp_arg)));
...
static esp_err_t my_ws_handler(httpd_req_t *req)
{
    if (req->method == HTTP_GET) {
        ESP_LOGI(TAG, "Handshake done, the new connection was opened");
        return ESP_OK;
...
void app_main(void)
{
    ...
    xTaskCreate(async_sender, "async_sender", 4096, NULL, configMAX_PRIORITIES-5, NULL);
}
}